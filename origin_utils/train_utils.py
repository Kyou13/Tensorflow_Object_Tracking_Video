...
def add_rectangles(H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25, show_suppressed=False):
    ...
    print("len(all_rects_r);{0}".format(len(all_rects_r)))
    if use_stitching:
        from stitch_wrapper import stitch_rects
        acc_rects = stitch_rects(all_rects, tau)
        print("first,len(acc_rects);{0}".format(len(acc_rects)))
    else:
    ...
    if show_suppressed:
        ...
        pairs = []
        # red
    print("len(acc_rects);{0}".format(len(acc_rects)))
    pairs.append((acc_rects, (0, 255, 0)))
    print("len(pairs);{0}".format(len(pairs)))
    for rect_set, color in pairs:
        c = 0
        # 2色のときのみ2loops
        for rect in rect_set:
            # many loops
            #if rect.confidence > min_conf and color == (0,255,0) :
